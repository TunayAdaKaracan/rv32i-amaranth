...
class ALU(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        with m.Switch(self.funct3):
            with m.Case(AluOp.ADD | AluOp.SUB):
            ...
            with m.Case(AluOp.SLL):
                m.d.comb += self.result.eq(self.value1 << shamt)
            with m.Case(AluOp.SLT):
                m.d.comb += self.result.eq(self.value1.as_signed() < self.value2.as_signed())
            with m.Case(AluOp.SLTU):
                m.d.comb += self.result.eq(self.value1 < self.value2)
